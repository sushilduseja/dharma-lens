...
const GenerateDynamicGuidanceOutputSchema = z.object({
  generated_pattern_name: z
    .string()
    .describe('A short, empathetic, and evocative name or title that encapsulates the essence of the user\'s situation (e.g., "Navigating Career Crossroads with a Heavy Heart", "Seeking Peace Amidst Family Tensions").'),
  generated_modern_context_and_insight: z
    .string()
    .describe('A concise (2-4 sentences) insight that acknowledges the user\'s feelings and provides a modern context or reframe for their situation. It should be empathetic and validating.'),
  generated_dharmic_guidance: z
    .array(z.string())
    .describe('3-4 practical, actionable, and compassionate dharmic guidance steps tailored to the described situation. Each step should be a single, clear sentence or at most two very short sentences. Focus on self-reflection, ethical decision-making, and fostering inner peace or constructive action.'),
...
2.  **Frame the Situation:** Create a 'generated_pattern_name'. This should be a short, empathetic, and evocative title that encapsulates the essence of what the user is going through.
3.  **Offer Insight:** Develop 'generated_modern_context_and_insight'. This should be 2-4 sentences. Start by acknowledging their feelings. Then, offer a modern context or a gentle reframe for their situation, helping them see it with a bit more clarity or compassion.
4.  **Provide Actionable Guidance:** Formulate 'generated_dharmic_guidance' consisting of 3-4 practical and compassionate steps. These steps should be:
...
5.  **Find a Sacred Verse (Optional but Valued):** If, and only if, you can find a Sanskrit shloka (or a verse from a Bhakti poet if more appropriate) that is *highly specific and profoundly relevant* to this unique situation, include it along with its English and Hindi translations.
    ...
    *   \\\`shlokaEnglishTranslation\\\`: Clear English.
    *   \\\`shlokaHindiTranslation\\\`: In Devanagari script.
...
*   **Conversational Language:** Use natural, clear, and kind language.
*   **Focus on Action and Reflection:** The guidance should empower the user to think or act constructively.

Generate a response strictly in the requested JSON format.
  `,
  config: {
    safetySettings: [
...
const generateDynamicGuidanceFlow = ai.defineFlow(
  ...
  async (input: GenerateDynamicGuidanceInput) => {
    ...
    // Ensure output is not null, if it could be, handle it.
    // For this setup, Genkit's definePrompt with output schema should ensure output matches or errors.
    return output!;
  }
);
