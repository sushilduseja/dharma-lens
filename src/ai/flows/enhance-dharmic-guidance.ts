...
const EnhanceDharmicGuidanceOutputSchema = z.object({
  enhancedGuidance: z
    .array(z.string())
    .describe('Enhanced dharmic guidance steps tailored to the user\'s situation. Aim for 3-4 impactful steps. Each step should be a single, clear, actionable sentence, or at most two very short sentences.'),
...
Provide enhanced guidance steps that are SUPER CONCISE, PUNCHY, and TO THE POINT.
- Offer specific, practical actions in alignment with Dharma, using impactful and direct language.
- Promote self-awareness, ethical decision-making, and a sense of purpose with brevity.
- Draw upon the depth of Hindu wisdom to address contemporary challenges, fostering equanimity and compassion.
- Each step MUST be a single, clear, actionable sentence, or at most two very short, impactful sentences.
...
- shlokaEnglishTranslation: Provide a clear English translation.
- shlokaHindiTranslation: Provide this in Devanagari script (for Hindi verses or as a translation for Sanskrit).
  `, config: {
    safetySettings: [
      {
